...
"""
代码功能：
    构建一个 GoogleNet 模型，在鱼类数据集上训练分类任务
"""
...
from lesson.C_GoogleNet_v1.tools.my_dataset import NCFMDataSet
from lesson.C_GoogleNet_v1.tools.common_tools import get_googlenet
...
if __name__ == "__main__":
    ...
    # config
    data_dir = os.path.join(BASE_DIR, "..", "..", "Data", "NCFM", "train")   ## 内有8个文件夹(8类)
...
# ============================ step 5/5 训练 ============================
    ...
    for epoch in range(start_epoch + 1, MAX_EPOCH):
        ...
        for i, data in enumerate(train_loader):
            ...
            # 2. backward
            optimizer.zero_grad()
            loss_main, aug_loss1, aug_loss2 = criterion(outputs[0], labels), criterion(outputs[1], labels), criterion(outputs[2], labels)
                                    ## 因为 googleNet 有两个辅助损失
            loss = loss_main + (0.3 * aug_loss1) + (0.3 * aug_loss2)    ## 设置辅助损失的权重为 0.3
            loss.backward()
